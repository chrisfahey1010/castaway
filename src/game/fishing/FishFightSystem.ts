...
export class FishFightSystem {
  ...
  update(
    ...
    fight.stamina = Math.max(0, fight.stamina - state.tension * 0.00013);
    fight.strength = Math.max(0, fight.strength - state.tension * 0.00010);
    ...
    if (state.elapsed >= GAME_CONFIG.fishing.maxFightSeconds / (1 + fight.erraticness * 2.5)) {
      return "escaped";
    }
